...
export const Accordion = ({ title, children, className, defaultOpen = false }: Props) => {
  const [isOpen, setIsOpen] = useState(defaultOpen);

  return (
    <div className={className}>
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center gap-2 p-2 rounded hover:bg-neutral-700/50 transition-colors">
