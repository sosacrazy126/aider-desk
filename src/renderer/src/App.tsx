...
const App = () => {
  const [openProjects, setOpenProjects] = useState<Project[]>([
    { baseDir: '/home/wladimiiir/Projects/aider-desktop' },
    { baseDir: '/home/wladimiiir/Projects/aider' },
  ]);
  ...
  const renderProjectPanels = () =>
    openProjects.map((project) => (
      <div key={project.baseDir} className="absolute top-0 left-0 w-full h-full" style={{ display: activeTab === project.baseDir ? 'block' : 'none' }}>
        <ProjectPanel baseDir={project.baseDir} />
      </div>
    ));
    ...
    <div className="flex flex-col h-screen">
      <div className="flex border-b">
        {openProjects.map((project) => (
          <button
            key={project.baseDir}
            className={`text-sm px-4 py-2 border-l-2 border-neutral-700 first:border-l-0 ${activeTab === project.baseDir ? 'bg-neutral-700 text-neutral-100' : 'bg-neutral-800 text-neutral-400 hover:bg-neutral-700 hover:text-neutral-200'}`}
